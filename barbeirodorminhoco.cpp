...
void* func_barbeiro(void *v) {
    ...
    while(1) {
        ...
        printf(ANSI_COLOR_CYAN "[BAR]->" ANSI_COLOR_RESET " Barbeiro %d cortou cabelo de um cliente. \n", id);  
...
void* func_cliente(void *v) {
    ...
    if (sem_trywait(&sem_cadeiras) == 0) {                      // tenta entrar e sentar 
        printf(ANSI_COLOR_GREEN "[CLI]->" ANSI_COLOR_RESET " Cliente %d entrou na barbearia.\n", id);        
        ...
        printf(ANSI_COLOR_YELLOW "[CLI]->" ANSI_COLOR_RESET " Cliente %d sentou na cadeira do barbeiro %d. \n", id, minha_cadeira);
        ...
        printf(ANSI_COLOR_RED "[CLI]->" ANSI_COLOR_RESET " Cliente %d deixou a barbearia.\n", id);
    } else {
        printf(ANSI_COLOR_BLUE "[CLI]->" ANSI_COLOR_RESET " Cliente %d não entrou na barbearia.\n", id);    // nao tem cadeiras vagas
    }
    return NULL;
}
